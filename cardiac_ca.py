...
class Config:
    ...
    # Stimulus placement
    stimulus_type: str = "lat_mask"  # "bottom_cap" or "lat_mask"
    stimulus_size: int = 60
    stimulus_data_path: str | None = "single_CL_LAT.dat"
    stimulus_value: float = 1.0
...
def _build_threshold_matrix(cfg: Config) -> np.ndarray:
    """Construct a uniform threshold matrix."""
...
def initialise_state(cfg: Config) -> tuple[SimState, np.ndarray]:
    ...
    elif stim_type == "lat_mask":
        if not cfg.stimulus_data_path:
            raise ValueError("stimulus_data_path must be set when using 'lat_mask'.")
        lat_values = np.loadtxt(cfg.stimulus_data_path, dtype=np.float32)
        if lat_values.ndim == 1:
            if lat_values.size != grid_shape[0] * grid_shape[1]:
                raise ValueError(
                    f"Expected {grid_shape[0] * grid_shape[1]} values, got {lat_values.size}."
                )
            lat_map = lat_values.reshape(grid_shape)
            lat_map = np.flipud(lat_map)
        elif lat_values.shape == grid_shape:
            lat_map = lat_values
        else:
            raise ValueError(
                f"stimulus data shape {lat_values.shape} does not match grid {grid_shape}."
            )
        stim_mask = lat_map == cfg.stimulus_value
...
# this is a specialised stopping condition 
...
def main() -> None:
    cfg = Config(
        stimulus_type="lat_mask",
        stimulus_data_path="single_CL_LAT.dat",
    )
