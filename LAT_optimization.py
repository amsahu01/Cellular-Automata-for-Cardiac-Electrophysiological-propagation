...
TOL_X = 1e-3              # Nelder–Mead parameter tolerance
TOL_F = 1.0               # Nelder–Mead objective tolerance (in squared-rel units)
PENALTY_LAT_MS = 1_000_000.0  # LAT assigned to cells that never activate
...
def build_objective(cfg: Config, mono_lat_ms: NDArray[np.float64], eps_ms: float, verbose: bool):
    ...
    def objective(raw_params: NDArray[np.float64]) -> float:
        ...
        if verbose:
            print(
                f"[eval {report_state['count']:04d}] sigma_trans={sigma_trans:7.4f}, "
                f"aniso_ratio={aniso_ratio:7.4f}, "
                f"threshold={threshold:7.4f} --> J={error_value:.3f}"
            )
        return error_value
...
def main() -> None:
    ...
    objective = build_objective(cfg, mono_lat_ms, EPS_MS, PRINT_EACH_EVAL)
    result = minimize(
        objective,
        simplex_raw[0],
        method="Nelder-Mead",
